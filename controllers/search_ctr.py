...
def search(cookies, query, category):
    data = ([(u, n, a, e, p) for u, n, a, e, p in get_users() if by_category(query, category, u, n, a, e, p) and a == cookies.get('info')[2]] 
            if cookies.get('roles')['is_dean'] or cookies.get('roles')['is_admin']
            else [(u, n, a, e, p) for u, n, a, e, p in get_users() if by_category(query, category, u, n, a, e, p)])
...
def get_users():
    return db.query('select UserName, Name, Area, email, phone from Users')